"""Isolated Phase 6 loop: Phase 5 mechanics plus mandatory output-safety restriction.

Intentional differences: versioned state/events, safety stage and coverage, strict
telemetry accounting, and terminal HUMAN_REVIEW. Baseline routing/gates are reused.
Keep parity tests when changing this adaptation; never modify Phase 5 to fit it.
"""
import copy
import math
import time
...
from orchestration.policy import WorkflowStop, SAFE_PROVIDER_CODES
from .policy import Phase6Policy as WorkflowPolicy, SAFE_WORKFLOW_CODES
from .routing import route, require_transition
from orchestration.state import DiagnosticVersion, CriticVersion
from .state import Phase6WorkflowState as WorkflowState, Phase6Validation as ValidationOutcome, Phase6Invocation as InvocationTelemetry
from safety import SafetyValidator
from safety.models import SafetyInput, SemanticSafetyResult
from safety.policy import deterministic_findings, make_assessment, validate_assessment
...
class Phase6Orchestrator:
    def __init__(self, llm: StructuredLLM, patient_rag, medical_rag, *, top_k=1,
        ...
        self.max_input_bytes = getattr(config, 'max_input_bytes', 100_000)
        self.byte_reserve = self.max_input_bytes * self.attempt_reserve
        ...
        if type(self.max_input_bytes) is not int or self.max_input_bytes < 1:
            raise ValueError('Provider must declare a positive byte limit')
    ...
    def _invoke(ticket, operation):
        ...
        return Phase6Response(ticket=ticket, generation=operation())
    ...
    def run(self, patient: PatientRepresentation, patient_id: str) -> WorkflowState:
        ...
        def event(name, reason, *, stage=None, valid=None, version=None):
            ...
            entry = WorkflowEvent(sequence=len(state.transition_history) + 1, timestamp=timestamp(),
                ...
                critic_fingerprint=safety_identity.critic_fingerprint if safety_identity else None,
                safety_input_fingerprint=safety_identity.input_fingerprint if safety_identity else None,
                safety_policy_version=safety_identity.safety_policy_version if safety_identity else None,
                safety_decision=state.safety_assessments[-1].decision if state.safety_coverage == 'assessed' else None,
                finding_count=len(state.safety_assessments[-1].findings) if state.safety_coverage == 'assessed' else None)
        ...
        def generate(expected, operation, schema):
            ...
            # Provider-supplied accounting is a contract, not optional best-effort data.
            if type(telemetry) is not list or type(attempts) is not int or len(telemetry) != attempts:
                raise WorkflowStop('invalid_provider_telemetry')
            for number, item in enumerate(telemetry, 1):
                if (type(item) is not dict or type(item.get('attempt')) is not int or item['attempt'] != number or
                        type(item.get('request_bytes')) is not int or not 1 <= item['request_bytes'] <= self.max_input_bytes):
                    raise WorkflowStop('invalid_provider_telemetry')
            if generation.failure is None and (attempts == 0 or telemetry[-1].get('api_success') is not True or
                                                telemetry[-1].get('accepted') is not True):
                raise WorkflowStop('invalid_provider_telemetry')
            observations = safe_observations(telemetry)
        ...
        def finish(stage, outcome, reason):
            status, name = {'FINAL': ('final', 'finalized'), 'ABSTENTION': ('abstained', 'abstained'),
                ...
                'HUMAN_REVIEW': ('human_review_required', 'human_review_required'),
        ...
        try:
            ...
            for revision in range(self.policy.max_revisions + 1):
                version = revision + 1
                safety_identity = None
                commit(safety_coverage='not_assessed', safety_skip_reason='awaiting_safety')
                if revision == 0:
                ...
                def current_safety_input():
                    # Recheck immutable evidence and all original patient/grounding gates.
                    validate_patient(state.patient_state, supplied)
                    latest, review = state.diagnostics[-1], state.critiques[-1]
                    expected_review = StageTicket(run_id=state.run_id, patient_id=patient_id, stage='CRITIC',
                        revision_number=revision, diagnostic_version=version,
                        evidence_snapshot_id=state.evidence.snapshot_id, diagnostic_fingerprint=latest.fingerprint)
                    require_current(expected_review, review.ticket)
                    if (latest.version != version or review.diagnostic_version != version or
                            review.evidence_snapshot_id != state.evidence.snapshot_id or
                            latest.fingerprint != fingerprint(latest.result.model_dump())):
                        raise WorkflowStop('stale_result')
                    cases, medical = state.evidence.agent_evidence()
                    value = validate_input(SafetyInput(patient_state=state.patient_state.model_copy(deep=True),
                        diagnostic=latest.result.model_copy(deep=True), critique=review.result.model_copy(deep=True),
                        patient_cases=cases, medical_knowledge=medical))
                    identity = SafetyTicket(run_id=state.run_id, patient_id=patient_id,
                        revision_number=revision, diagnostic_version=version,
                        diagnostic_fingerprint=latest.fingerprint, evidence_snapshot_id=state.evidence.snapshot_id,
                        critic_fingerprint=fingerprint(review.result.model_dump()),
                        input_fingerprint=fingerprint(value.model_dump()))
                    return value, identity

                safety_input, safety_identity = timed('SAFETY_INPUT', current_safety_input)
                event('safety_started', 'mandatory_safety_gate', stage='SAFETY_VALIDATION', version=version)
                timed('SAFETY_DETERMINISTIC', lambda: deterministic_findings(safety_input))
                event('safety_deterministic_completed', 'deterministic_checks_passed', valid=True, version=version)
                budget()
                semantic = None
                if not safety_input.critique.safety_flags:
                    if self.safety is None or not callable(getattr(self.safety, 'run', None)):
                        raise WorkflowStop('safety_validator_unavailable')
                    semantic = generate(safety_identity, lambda: self.safety.run(safety_input.model_copy(deep=True)),
                                        SemanticSafetyResult)
                    # Mandatory application validation even if a fake provider skipped its callback.
                    try:
                        semantic = timed('SAFETY_RESULT_VALIDATION', lambda: validate_semantic(semantic, safety_input))
                    except (ValueError, TypeError):
                        raise WorkflowStop('safety_validation_failure') from None
                    event('safety_semantic_completed', 'semantic_result_validated', valid=True, version=version)
                # Reconstruct from committed state after the handoff, never trust caller mutations.
                fresh_input, fresh_identity = timed('SAFETY_INPUT', current_safety_input)
                require_current(safety_identity, fresh_identity)
                try:
                    assessment = timed('SAFETY_POLICY', lambda: make_assessment(fresh_input, fresh_identity, semantic))
                    timed('SAFETY_POLICY', lambda: validate_assessment(assessment, fresh_input, fresh_identity))
                except (ValueError, TypeError):
                    raise WorkflowStop('safety_validation_failure') from None
                budget()
                commit(safety_assessments=state.safety_assessments + (assessment,),
                       safety_coverage='assessed', safety_skip_reason=None)
                validation(True, None, 'safety_validated')
                event('safety_validated', 'assessment_committed', valid=True, version=version)
                event('safety_decision_recorded', 'application_safety_policy', valid=True, version=version)
                budget()
            ...
            if state.safety_coverage != 'assessed':
                in_safety = state.current_stage in {'SAFETY_VALIDATION', 'ROUTE'}
                commit(safety_coverage='failed' if in_safety else 'not_assessed',
                       safety_skip_reason='safety_validation_failed' if in_safety else 'technical_failure_before_safety')
